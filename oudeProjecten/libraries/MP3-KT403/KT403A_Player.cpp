...
again:
  ...
  if (commandBytes[1] == KT403A_SET_DEVICE)
  { // if the cmd is set device.
    delay(200);
  }
...
// feedback on
  ...
  while (_serial->available() && ((millis() - time) < KT403A_TIMEOUT))
  {
    ...
    /*
      // ========= Harm lezen uit serial read??
      for (int x=0;x<30;x++) {
        SerialUSB.print(x);
        SerialUSB.print(" : ");
        SerialUSB.println(recv[x]);
      } 
      // ======= einde iegen stukje 
      */
...
/**************************************************************** 
 Harms functie
 Print de teruggekomen data van de MP3 module
****************************************************************/
template <class T>
uint8_t KT403A<T>::speeltNog(void)
{
  {
      int speelData = _serial->read(); //roept de functie 'read()' aan in de '_serial' namespace

      // Serial.print("speelData : ");
      // Serial.println(speelData);

      if (speelData==60 || speelData==61 || speelData==62)
      {      
        geluidGestopt = true;
      }
  //  }
...
/**************************************************************** 
Bijhouden in een toggle of het geluid gestart is of gestopt is
****************************************************************/
template <class T>
uint8_t KT403A<T>::speeltToggle(void)
{
speeltNog(); // roep de functie aan die bekijkt of het geluid is getopt
